...
from odoo.addons.website_support_ticket_odoo_aagam.models.aagam_helpdesk_ticket import TICKET_PRIORITY



class HelpdeskTicketSla(models.Model):
    _name = "aagam.helpdesk.ticket.sla.policy"
    _description = "Helpdesk Ticket SLA Policy"

    name = fields.Char(string="Name", required=True)
    helpdesk_stage_id = fields.Many2one("aagam.helpdesk.stage", string="Stage")
    ...
    exclude_stage_ids = fields.Many2many(
        'aagam.helpdesk.stage', string='Exclude Stages',
        compute='_compute_exclude_stage_ids', store=True, readonly=False, copy=True,
        domain="[('id', '!=', helpdesk_stage_id.id)]")
    team_id = fields.Many2one('aagam.helpdesk.ticket.team', 'Team', required=True)
    ticket_type_id = fields.Many2one(
        'aagam.helpdesk.ticket.type', "Ticket Type")
    tag_ids = fields.Many2many(
        'aagam.helpdesk.ticket.tag', string='Tags')
...
class HelpdeskSLAStatus(models.Model):
    ...
    ticket_id = fields.Many2one('aagam.helpdesk.ticket', string='Ticket', required=True, ondelete='cascade', index=True)
    sla_id = fields.Many2one('aagam.helpdesk.ticket.sla.policy', required=True, ondelete='cascade')
    sla_stage_id = fields.Many2one('aagam.helpdesk.stage', related='sla_id.helpdesk_stage_id', store=True)  # need to be stored for the search in `_sla_reach`
